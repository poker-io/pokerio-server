...
test('Kick player, wrong args', (done) => {
  ...
  request(app)
    .get('/kickPlayer?playerToken=1337')
    .expect(400)
    .end(done)
...
test('Kick player, correct arguments', async () => {
  ...
  await request(app) // Creator exists, but the player does not
    .get('/kickPlayer?creatorToken='.concat(gameMasterToken).concat('&playerToken=2137'))
    .expect(400)
  ...
  const getNewSmallBlindCurrentPlayer = 'SELECT small_blind_who FROM Games WHERE game_id=$1'
  const verifyMinQuery = 'SELECT token FROM Players WHERE token=$1 AND game_id=$2 AND turn IN (SELECT MIN(turn) FROM Players WHERE game_id=$2)'
  await client
    ...
    .then(async (result) => {
      ...
      await request(app).get('/kickPlayer?'.concat('creatorToken='.concat(gameMasterToken).concat('&playerToken='.concat(playerToken)))).expect(200)

      await client.query(verifyNoPlayerQuery, [playerToken]).then((res) => {
        expect(res.rowCount).toEqual(0)
      })

      await client.query(getNewSmallBlindCurrentPlayer, [gameId]).then(async (newSmallest) => {
        await client.query(verifyMinQuery, [newSmallest.rows[0].small_blind_who, gameId]).then((res) => {
          console.log(newSmallest)
          console.log(res)
          expect(newSmallest.rows[0].small_blind_who).toEqual(res.rows[0].token)
        })
      })
    })
    .finally(async () => {
